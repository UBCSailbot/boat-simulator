import numpy as np
from boat_simulator.nodes.physics_engine.medium_force_computation import MediumForceComputation
import pytest
import math
...
@pytest.mark.parametrize("orientation, expected_lift, expected_drag, apparent_velocity", [
    ...
    (0, 11768, 3249, np.array([44*math.cos(np.deg2rad(20)), 44*math.sin(np.deg2rad(20))])),

])
def test_compute_forces(medium_force_setup, orientation, expected_lift, expected_drag, apparent_velocity):
    ...
    assert np.isclose(calculate_magnitude(lift_force), expected_lift, rtol=.05)
    assert np.isclose(calculate_magnitude(drag_force), expected_drag, rtol=.05)
